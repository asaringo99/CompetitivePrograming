...
int main(){
    ...
    ll all = powmod(sum,mod-2) ;
    dp[0][0][0] = 1 ;
    rep(i,n) rep(j,k+1) rep(x,i+1) {
        (dp[i+1][j][x] += dp[i][j][x]) %= mod ;
        rrep(a,1,k+1){
            if(j + a > k) break ;
            (dp[i+1][j+a][x+1] += dp[i][j][x] * powmod(permutation(a,a),mod-2) % mod * powmod(W[i]*all%mod,a) % mod) %= mod ;
        }
    }
    cout << dp[n][k][m] * permutation(k,k) % mod << endl ;
}